...
RSpec.describe Review, type: :model do
  describe 'validations' do
    ...
    let(:restaurant) { Restaurant.create(name: 'Test Restaurant', address: '123 Test Street') }

    it 'is not valid without a rating' do
      review = Review.new(user: user, restaurant: restaurant, rating: nil)
    ...
    it 'is not valid with a comment shorter than the minimum length' do
      review = Review.new(user: user, restaurant: restaurant, comment: 'Hi')
      expect(review).not_to be_valid
    ...
    it 'is valid with a comment within the length limit' do
      review = Review.new(user: user, restaurant: restaurant, comment: 'Great restaurant', rating: 4, review_images: [])
    ...
    it 'is not valid with a comment longer than the maximum length' do
      ...
      review = Review.new(user: user, restaurant: restaurant, comment: long_comment)
      expect(review).not_to be_valid
